package nghiem.nc.springbootoauth2keycloak.config;
...
public class JwtAccessTokenCustomizer extends DefaultAccessTokenConverter implements
  ...
  /**
   * Spring oauth2 expects roles under authorities element in tokenMap, but keycloak provides it
   * under resource_access. Hence extractAuthentication method is overriden to extract roles from
   * resource_access.
   *
  ...
  public OAuth2Authentication extractAuthentication(Map<String, ?> tokenMap) {
    ...
    OAuth2Request request =
        new OAuth2Request(oAuth2Request.getRequestParameters(), oAuth2Request.getClientId(),
            authorities, true, oAuth2Request.getScope(),
